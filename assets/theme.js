...
theme.Product = (function() {
    ...
    Product.prototype = _.assignIn({}, Product.prototype, {
        ...
        _updateAddToCart: function(evt) {
            var variant = evt.variant;
			$(this.selectors.addToCart).removeClass('preorder');
            if (variant) {
                if (variant.available) {
                    /* Preorder js */
                    if (typeof preorderarray !== 'undefined') {
                      var preflag = false;
                      preorderarray.forEach(function( item, index ) {
                        if( item.id == variant.id ){
                          if( item.available && item.inventory_quantity <= 0 && item.inventory_policy == "continue" ){
                            preflag = true;
                          }
                        }
                      });
                      if( preflag ){
                          $(this.selectors.addToCart).addClass('preorder');
                          $(this.selectors.addToCart).prop('disabled', false);
                          $(this.selectors.addToCartText).text(theme.strings.preOrderCart);
                    ...
                    if (theme.strings.templ == "product.comingsoon") {
