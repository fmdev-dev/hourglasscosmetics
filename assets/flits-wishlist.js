      ...
      item.addEventListener('click',function(e){
        ...
        switch(page){
          ...
          case "collection-page":
            ...
            /* start wishlist datalaer */
              window.dataLayer.push({
                'event': "addWishlist",
                'pageType': 'Product Detail Page'
              }); 
            /* end wishlist datalayer*/
          ...
          if (this.readyState == 4) {
            if (this.status == 200) {
              ...
              switch(page){
                ...
                case "account-wsl-page":
                  if(document.querySelector(".flits-remove-from-wsl-btn[data-product-id='"+product_id+"']")){
                    document.querySelector(".flits-remove-from-wsl-btn[data-product-id='"+product_id+"']").closest("div .flits-wsl-product-row").remove();
                    var removewsl = document.querySelectorAll(".flits-wsl-product-row");
                    if(removewsl.length < 1){
                      document.querySelector('.flits-wsl-table').style.display = "none";
  ...
  flitsAppJquery.ajax({
    ...
    success:function(data){
        ...
        if(status && count > 0){
          ...
          flitsAppJquery('.flits-wsl-view-empty').hide();
          flitsAppJquery('.flits-wishlist-ul').show();
  ...
  $_("[data-flits-wishlist-product-handle]").each(function(index,item){
    ...
    $_.ajax({
      ...
      success:function(data){
        ...
        $_(wishlist_product_selector).find('.flits-wsl-product-remove').attr('data-product-id',product.id);
        $_(wishlist_product_selector).find('.flits-wsl-product-remove').attr('data-product-title',product.title);
        $_(wishlist_product_selector).find('.flits-wsl-product-remove').attr('data-product-handle',product.handle);
...
if(location.pathname == "/account"){
  ...
  window.flitsApp.wishlist_products_interval = setInterval(function(){  // add it to if(account page)
    if (typeof flitsAppJquery == "undefined"){

    }
    else{
      clearInterval(window.flitsApp.wishlist_products_interval);
      window.flitsApp.get_wishlist_products_using_ajax();
    }
